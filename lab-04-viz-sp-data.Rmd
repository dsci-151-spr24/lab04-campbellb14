...
***Each row represents the data for one dennys restaurant. The variables are:
`r names(dennys)`.
...
***Each row represents the data for one La Quintas Inns The variables are:
`r names(laquinta)`.
...
***There are La Quinta locations outside of the US but they're mostly located in the Americas. They're located in Mexico, Canada, New Zealand, China, Turkey, etc.
Denny's is only located in the US.*** 
...
dennys %>%
  ...
  arrange(desc(n))
```
***California has the most Dennys locations, Delaware has the fewest. This isn't surprising because California is a large and populated state whereas Delaware is a very small state.***
...
laquinta %>%
  ...
  arrange(desc(n))
```
***Texas has the most La Quinta locations, Maine has the fewest. This isn't surprising because Texas is a large and populate state whereas Maine is smaller and less populated.***
...
laquinta %>%
  ...
  arrange(desc(per_thousand))
```
***DC has the most Denny's per square mile and Rhode Island has the most La Quinta's per square mile.***
